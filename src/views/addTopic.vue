        ...
        <div class="main-container wrap backgroundModule main-fullScreen">
            <div class="addTopicModule">
                ...
                >
                    <el-row>
                            ...
                            <el-form-item :error="error.tagId">
                                ...
                                >
                                    <el-option
                                        ...
                                        :label="item.label"
                                        :value="item.value"
                                        :disabled="!isAllowTag(item.value)"
                                    ></el-option>
                                </el-select>
                            </el-form-item>
                        </el-col>
                        ...
                        <el-col :span="1" :offset="2">
                            <div
                                class="redEnvelopeButton"
                                v-if="form.showRedEnvelopeText"
                                @click="redEnvelopeForm"
                            >
                    ...
                    <div v-if="form.showRedEnvelopeForm">
                        <el-form-item>
                            <el-radio-group
                                v-model="form.giveRedEnvelope_type"
                                @change="selectRedEnvelopeType()"
                            >
                        ...
                        <el-form-item
                            ...
                            v-if="form.giveRedEnvelope_type == 20"
                        >
                            红包总金额
                            <el-input
                                v-model.trim="form.giveRedEnvelope_totalAmount"
                                placeholder="输入金额"
                                maxlength="12"
                                @input="totalAmountCalculate()"
                                clearable
                                style="width: 150px; margin-left: 10px; margin-right: 10px"
                            ></el-input
                            >元
                            <span class="help-text">
                                <span v-if="parseFloat(form.giveRedEnvelopeAmountMin) > 0"
                                    >合计总金额不能少于
                                    {{ form.giveRedEnvelopeAmountMin }} 元；</span
                                >
                                <span
                                    v-if="
                                        form.giveRedEnvelopeAmountMax != null &&
                                        parseFloat(form.giveRedEnvelopeAmountMax) > 0
                                    "
                                    >合计总金额不能超过
                                    {{ form.giveRedEnvelopeAmountMax }} 元；</span
                                >
                                <span>用户共有金额 {{ form.deposit }} 元</span>
                            </span>
                        </el-form-item>
                        <el-form-item
                            :error="error.singleAmount"
                            v-if="form.giveRedEnvelope_type == 30"
                        >
                            单个红包金额
                            <el-input
                                v-model.trim="form.giveRedEnvelope_singleAmount"
                                placeholder="输入金额"
                                maxlength="12"
                                @input="singleAmountCalculate()"
                                clearable
                                style="width: 150px; margin-left: 10px; margin-right: 10px"
                            ></el-input
                            >元
                        ...
                        <el-form-item :error="error.giveQuantity">
                            <el-input
                                v-model.trim="form.giveRedEnvelope_giveQuantity"
                                placeholder="输入数量"
                                maxlength="8"
                                @input="singleAmountCalculate()"
                                clearable
                        ...
                        <el-form-item :error="error.redEnvelopeLimit">
                            合计总金额<span class="totalAmountView">{{
                    ...
                    <el-form-item :error="error.content">
                        ...
                        <div v-if="form.isMarkdown" style="width: 100%">
                            <Editor
                                ...
                                :plugins="form.addTopicEditorPlugin"
                                :locale="zhHans"
                                :editorConfig="markdownEditorConfig"
                                :sanitize="addTopicSanitize"
                                placeholder="请输入内容..."
                    ...
                    <el-form-item
                        :error="error.captchaValue"
                        prop="captchaValue"
                        v-if="form.showCaptcha"
                        class="captcha-item"
                    >
                        <el-row>
                            <el-col :span="10">
                ...
                <div class="selectMentionUserModule">
                    <el-dialog
                        title="选择用户"
                        v-model="form.popup_mentionUser"
                        @close="closeMentionUser"
                        :draggable="true"
                        top="40vh"
                        width="500"
                    >
                        <div class="search">
                            <el-form
                                :inline="true"
                                label-width="auto"
                                @keyup.enter.native="searchMentionUser"
                            >
                        ...
                        <div
                            ...
                            v-for="mentionUser in form.mentionUserList"
                            @click="selectMentionUser(mentionUser)"
                        >
                            <div class="left-layout">
                                <div class="avatarBox">
                                    <img
                                        v-if="mentionUser.avatarName != null"
                                        :src="
                                            mentionUser.avatarPath +
                                            '100x100/' +
                                            mentionUser.avatarName
                                        "
                                        class="img"
                                    />
                                    <img
                            ...
                            <div class="right-layout">
                                <span class="userInfo">{{
                                    mentionUser.nickname != null && mentionUser.nickname != ''
...
const addTopicSanitize = (schema: any) => {
    ...
    if (form.availableTag?.indexOf('embedVideo') != -1) {
        //嵌入视频
...
const handleToggleRichtextEditor = (editorId: string) => {
    ...
    nextTick(() => {
        if (Object.keys(form.addTopicEditorCreateParameObject).length != 0) {
            //不等于空
            //创建富文本编辑器
        ...
        .then((response: AxiosResponse) => {
            if (response) {
                ...
                if (data.allowTopic) {
                    ...
                    if (data.availableTag != null && data.availableTag != '') {
                        ...
                        if (availableTagObject != null && availableTagObject.length > 0) {
                            for (let i = 0; i < availableTagObject.length; i++) {
                                ...
                                if (
                                    availableTag == 'hidePassword' ||
                                    availableTag == 'hideComment' ||
                                    availableTag == 'hideGrade' ||
                                    availableTag == 'hidePoint' ||
                                    availableTag == 'hideAmount'
                                ) {
                    ...
                    if (
                        data.giveRedEnvelopeAmountMax == null ||
                        (data.giveRedEnvelopeAmountMax != null &&
                            parseInt(data.giveRedEnvelopeAmountMax) > 0)
                    ) {
                        //显示发红包
                    ...
                    if (
                        form.addTopicEditorPlugin != null &&
                        form.addTopicEditorPlugin.length == 0
                    ) {
                        //添加插件
                        ...
                        if (form.availableTag?.indexOf('mention') != -1) {
                            form.addTopicEditorPlugin.push(
                                mention((editorId: string, ctx: BytemdEditorContext) => {
                                    mentionPopUp(editorId, ctx)
                                })
                            )
                        }

                        if (form.supportEditor == 30 || form.supportEditor == 40) {
                            //10.仅富文本编辑器 20.仅Markdown编辑器  30.富文本编辑器优先 40.Markdown编辑器优先
                            form.addTopicEditorPlugin.push(
                                toggleEditor((editorId: string) => {
                                    handleToggleRichtextEditor(editorId)
                                })
                            )
                        }
                        form.addTopicEditorPlugin.push(help(form.availableTag, form.userGradeList))

                        if (form.availableTag?.indexOf('insertfile') != -1) {
                            form.addTopicEditorPlugin.push(
                                fileUpload(
                                    'user/control/topic/upload?method=upload',
                                    'file',
                                    form.fileSystem
                                )
                            )
                        ...
                        if (
                            form.availableTag?.indexOf('hidePassword') != -1 ||
...
const selectRedEnvelopeType = () => {
    if (form.giveRedEnvelope_type == 20) {
        //随机
        totalAmountCalculate()
    } else if (form.giveRedEnvelope_type == 30) {
        //固定
...
const singleAmountCalculate = () => {
    ...
    if (form.giveRedEnvelope_type == 30) {
        //固定
        //金额 只允许正数  正则判断最多10位小数
        var exp_singleAmount =
            /(^[1-9]([0-9]+)?(\.[0-9]{1,10})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/
        //数量 正整数
        var exp_giveQuantity = /^\+?[1-9][0-9]*$/
        if (
            exp_singleAmount.test(form.giveRedEnvelope_singleAmount.trim()) &&
            exp_giveQuantity.test(form.giveRedEnvelope_giveQuantity.trim())
        ) {
            var amount = calc_multiply(
                form.giveRedEnvelope_singleAmount.trim(),
                form.giveRedEnvelope_giveQuantity.trim(),
                2
            )
            form.giveRedEnvelope_totalAmountView = String(amount)
        } else {
